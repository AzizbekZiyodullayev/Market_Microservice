...
import (
	...
	grpc "gitlab.com/market_mc/market_go_staff_service/grpc"
	"gitlab.com/market_mc/market_go_staff_service/grpc/client"
	"gitlab.com/market_mc/market_go_staff_service/pkg/logger"
	"gitlab.com/market_mc/market_go_staff_service/storage/postgres"
)
...
func main() {
	...
	// Listen Port
	lis, err := net.Listen("tcp", cfg.StaffGRPCPort)
	if err != nil {
		log.Panic("net.listen", logger.Error(err))
	}

	log.Info("GRPC: Server being started...", logger.String("port", cfg.StaffGRPCPort))
